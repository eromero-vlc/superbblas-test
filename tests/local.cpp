...
int main(void) {
    ...
    const Coor<Nd> dim = {16, 16, 16, 16, 16, 16};
    //using T = double;
...
#    pragma omp parallel for
        ...
        // Copy tensor t0 into each of the c components of tensor 1
        {
            ...
            t = omp_get_wtime() - t;
            std::cout << "Time in copying/permuting from xyzts to tcxyzs " << t / 10 << std::endl;
        }
        ...
        Tensor tc(volc);
        {
            double t = omp_get_wtime();
            for (unsigned int rep = 0; rep < 10; ++rep) {
                local_contraction<Nd, Nd, 3>("tcxyzs", dim, false, t1.data(), "tCxyzs", dim1, false,
                                             t2.data(), "tCc", dimc, tc.data(), ctx);
            }
            t = omp_get_wtime() - t;
...
#ifdef SUPERBBLAS_USE_CUDA
    {
        ...
        Tensor tc(volc);
        {
            double t = omp_get_wtime();
            for (unsigned int rep = 0; rep < 10; ++rep) {
                local_contraction<Nd, Nd, 3>("tcxyzs", dim, false, t1.data().get(), "tCxyzs", dim1,
